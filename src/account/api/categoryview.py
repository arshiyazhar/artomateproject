...
from rest_framework.views import APIView
from account.models import Categories, SubCategory, Const_skills, TicketCat, RaiseTicket,PostProject,country
from account.api.serializers import CategoriesSerializer, SubCategorySerializer, RaiseTicketSerializer
...
class AllCategories(APIView):
    def get(self, request):
        mylist = []
        data1 = {}
        allcategories = Categories.objects.all().values('id', 'category_name', 'category_image','category_code')
        data1['categories'] = allcategories             
             
        return Response(data1)
        
class CategoriesWithJobs(APIView):
    def get(self, request):
        mylist = []
        data1 = {}
        allcategories = Categories.objects.all().values('id', 'category_name', 'category_image','category_code')
        for i in allcategories:
            ...
            data = {"categories": i, "no_of_posted_jobs": projects}
            mylist.append(data)
        data1['data'] = mylist
...
class CategoryList(APIView):
    def get(self, request, cat_id):
        if request.method == 'GET':
            ...
            skills = Const_skills.objects.filter(category_id=cat_id).values('id', 'skill_name')
            print(skills)
            mylist = []
            mylist.append(sub)
