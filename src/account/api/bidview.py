...
class Bid_Details_Project(APIView):
    ...
    def get(self, request, projectroute):
        ...
        data1={}
        mylist=[]
        projects_posted = PostProject.objects.filter(route=projectroute).filter(userid=id).values('id', 'route', 'project_deadline','min', 'max','project_title')
        if projects_posted.exists():
            ...
            for i in projects_posted:
                ...
                biddeatils =Bidproject.objects.filter(project_id=i['id']).values('bid_amount' ,'user_id','completion_time','email','descreption')
                print(biddeatils)
                for j in biddeatils:
                    ...
                    for k in userdetails:
                        ...
                        data={ "username":k['user_name'],"profile":k['profile'],"location":country.objects.filter(id=k['country_id']).values('country_name'),
                               "email":j['email'],"bidamount":j['bid_amount'],"review":3,"hourely":k['hourely_rate'],"completiontime":j['completion_time'] ,"description":j['descreption'], "fullname":fullname}
            ...
            for i in projects_posted:
                ...
                data1['project_name']=i['project_title']
                data1['project_route']=i['route']
...
class No_Of_Bid(APIView):
    ...
    def get(self, request):
        ...
        biddetails = Bidproject.objects.filter(user_id=id).values('bid_amount', 'user_id', 'completion_time',
                                                                            'email', 'created_at','project_id','descreption')
        ...
        for var in biddetails:
            ...
            for i in projectdetails:
                data={
                    ...
                    "completiontime":var['completion_time'],"bid_description":var['descreption']
